...
class CharacterBehavior(BehaviorScript):
    """
    Character controller for male_adult_construction_05.

    The character repeatedly moves between:
    - start position: (6.0, 0.8, 0.0)
    - end position:   (1.3, 0.8, 0.0)
    """
    ...
    def renew_character_state(self):
        ...
        """
        self.character_name = "male_adult_construction_05"
    ...
    def get_simulation_commands(self):
        # Explicitly use both fixed endpoints so the route does not depend on
        # the character's transform when simulation starts.
        return [
            ["GoTo", "6.0", "0.8", "0.0", "0"],
            ["GoTo", "1.3", "0.8", "0.0", "0"],
        ]
