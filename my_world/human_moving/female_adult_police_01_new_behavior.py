...
class CharacterBehavior(BehaviorScript):
    """
    Character controller for female_adult_police_01.

    The character repeatedly moves between:
    - start/origin position: (7.0, -3.0, 0.0)
    - waypoint:              (7.0,  1.0, 0.0)
    - target position:       (1.0,  1.0, 0.0)
    """
    ...
    def renew_character_state(self):
        ...
        """
        self.character_name = "female_adult_police_01"
    ...
    def get_simulation_commands(self):
        # init_character() appends a GoTo back to the original position, so
        # these commands create:
        # origin -> waypoint -> target -> waypoint -> origin
        return [
            ["GoTo", "7.0", "1.0", "0.0", "0"],
            ["GoTo", "1.0", "1.0", "0.0", "0"],
            ["GoTo", "7.0", "1.0", "0.0", "0"],
